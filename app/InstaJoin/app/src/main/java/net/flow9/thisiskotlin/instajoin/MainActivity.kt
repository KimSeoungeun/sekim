...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.login_btn).doAfterTextChanged {
            val user = HashMap<String,Any>()
            user.put("username", username)
            user.put("password", passwoed)
            retrofitService.instaLogin(user).enqueue(object : Callback<User>{
                override fun onResponse(call: Call<User>, response: Response<User>) {
                    if(response.isSuccessful){
                        ...
                        editor.commit()
                        startActivity(Intent(this@MainActivity,InstaMainActivity::class.java))
                    }
                }
